...
try {
    ...
    if (isset($_FILES['archivo'])) {
        ...
        foreach ($files['name'] as $key => $value) {
            $archivo = str_replace(' ', '_', $files['name'][$key]); 
            $archivo_tmp = $files['tmp_name'][$key];
            $archivo_ruta = $carpetaRuta. '/'. $archivo;

            if (move_uploaded_file($archivo_tmp, $archivo_ruta)) {
                $mensaje = "Archivo '$archivo' subido con éxito.";
            } else {
                $mensaje = "Error al subir el archivo '$archivo'.";
            }
        }
    }
    ...
    if (isset($_POST['eliminarArchivo'])) {
        ...
        if (file_exists($archivoRutaAEliminar)) {
            if (is_dir($archivoRutaAEliminar)) {
                // Eliminar directorio
                eliminarDirectorio($archivoRutaAEliminar);
                $mensaje = "Directorio '$archivoAEliminar' eliminado con éxito.";
            } else {
                // Eliminar archivo
            ...
            throw new Exception("El archivo o directorio '$archivoAEliminar' no existe.");
        }
    }
} catch (Exception $e) {
    ...
    <div class="content">
        ...
        <div class="container">
            <div class="drop-area" id="drop-area">
                ...
                <button type="submit" class="btn-enviar">ENVIAR ARCHIVO</button>
            </form>
        </div>
